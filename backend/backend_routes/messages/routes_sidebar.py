...
async def get_conversation_sidebar(
    ...
    peer_user_id: str = Query("", max_length=120),
):
    ...
    # Profile subject: other party when provided, else seller (company/employer)
    subject_id = peer or seller
    user_doc = await _find_user_by_identifier(subject_id)
    brand = await _resolve_company_branding(subject_id, {})
    preference_doc = await message_conversation_preferences_collection.find_one(
        {"seller_user_id": seller, "viewer_user_id": viewer}
    ) or {}

    filters = {"seller_user_id": {"$regex": f"^{seller}$", "$options": "i"}}
    ...
    display_name = brand.get("name") or _pick_user_name(user_doc) or subject_id
    imageurl = brand.get("imageurl") or _pick_user_avatar(user_doc)
    online_status = _presence_from_user(user_doc)
    ...
    return {
        ...
        "profile": {
            ...
            "country": str(
                ...
                or user_doc.get("location_country")
