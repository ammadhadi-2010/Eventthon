"""Session headers and squad leadership checks."""
...
async def verify_squads_session(
...
async def assert_squad_leader(squad: dict, actor_id: str, user: dict) -> None:
    ...
    if leader and leader != actor_id.strip():
        raise HTTPException(status_code=403, detail="Only the squad leader can perform this action")
